...
const addresses = ref([]); const selectedAddressId = ref(null); const remark = ref(''); const shopName = ref(''); const items = ref([])
...
async function load() {
  ...
  try {
    ...
    const [cart, addressList] = await Promise.all([getCart(), getAddresses()])
    ...
    shopName.value = groups[0].shopName; items.value = groups[0].items; addresses.value = addressList || []
...
async function submit() {
  ...
  try {
    const result = await createOrder({ cartItemIds: cartItemIds.value, addressId: selectedAddressId.value, remark: remark.value || undefined })
...
<template>
  <section class="page-shell"><PageHeader title="结算" description="这里展示购物车预估金额；提交后以服务端重新校验并返回的金额为准。" eyebrow="CHECKOUT" /><AsyncState :loading="loading" :error="error" :empty="!items.length" empty-text="没有可结算的购物车商品" @retry="load"><template #empty-action><el-button type="primary" @click="router.push('/cart')">返回购物车</el-button></template><div class="checkout-grid"><main><el-card class="ui-card"><template #header>收货地址</template><el-radio-group v-if="addresses.length" v-model="selectedAddressId" class="address-list"><el-radio v-for="address in addresses" :key="address.id" :label="address.id"><strong>{{ address.receiverName }}</strong><span>{{ address.maskedReceiverPhone }}</span><p>{{ address.formattedAddress }}</p></el-radio></el-radio-group><el-empty v-else description="暂无校园地址"><el-button type="primary" @click="router.push('/profile/addresses')">去管理地址</el-button></el-empty></el-card><el-card class="ui-card"><template #header>{{ shopName }}</template><div v-for="item in items" :key="item.cartId" class="checkout-item"><span>{{ item.goodsName }} × {{ item.quantity }}</span><strong>¥{{ money(item.subtotal) }}</strong></div><el-form-item label="订单备注"><el-input v-model="remark" maxlength="255" show-word-limit placeholder="选填，商家可见" /></el-form-item></el-card></main><aside><el-card class="ui-card"><div class="amount-row"><span>商品金额</span><strong>¥{{ money(totalAmount) }}</strong></div><div class="amount-row"><span>配送费</span><span>¥0.00</span></div><div class="amount-row"><span>优惠金额</span><span>-¥0.00</span></div><div class="pay-row"><span>应付金额</span><strong>¥{{ money(totalAmount) }}</strong></div><el-button type="primary" size="large" :loading="submitting" :disabled="!items.length || !selectedAddressId || submitting" @click="submit">提交订单</el-button></el-card></aside></div></AsyncState></section>
</template>
