...
import { cancelOrder, getOrderDetail, getOrders, remindOrder, simulateOrderPayment } from '../api/order'
...
async function remind(row) { try { operating.value = row.orderId; await remindOrder(row.orderId); ElMessage.success('催单已发送') } catch (error) { ElMessage.error(error.message || '催单发送失败') } finally { operating.value = null } }
