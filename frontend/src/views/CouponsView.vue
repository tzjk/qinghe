...
async function claim(item) { claiming.value = item.id; try { await claimCoupon(item.id); ElMessage.success('领取成功'); await Promise.all([loadAvailable(), loadMine()]) } catch (error) { ElMessage.error(error.message || '领取失败') } finally { claiming.value = null } }
...
<template>
  <section class="page-shell"><PageHeader title="优惠券" description="领取与使用均以服务端时间、库存和订单金额校验为准。" eyebrow="COUPONS" />
    <el-tabs v-model="active" @tab-change="changeTab"><el-tab-pane label="可领取" name="available"><div v-loading="loading" class="coupon-grid"><el-empty v-if="!available.length" description="暂无可领取优惠券" /><el-card v-for="item in available" :key="item.id" class="coupon-card"><h3>{{ item.name }}</h3><strong>{{ description(item) }}</strong><p>满 ¥{{ money(item.thresholdAmount) }} 可用 · 适用店铺 #{{ item.shopId }}</p><small>领取：{{ item.receiveStartTime }} 至 {{ item.receiveEndTime }}</small><small>使用：{{ item.useStartTime }} 至 {{ item.useEndTime }}</small><el-button type="primary" :loading="claiming === item.id" @click="claim(item)">领取</el-button></el-card></div></el-tab-pane>
