...
public class ExploreServiceImpl implements ExploreService {
    ...
    public ExploreServiceImpl(ExplorePostMapper postMapper, ExploreLikeMapper likeMapper, ExploreCommentMapper commentMapper, ShopMapper shopMapper, UserMapper userMapper, ExploreHotService hotService, StringRedisTemplate redisTemplate, AliyunOSSOperator ossOperator) {
        this.postMapper = postMapper; this.likeMapper = likeMapper; this.commentMapper = commentMapper; this.shopMapper = shopMapper; this.userMapper = userMapper; this.hotService = hotService; this.redisTemplate = redisTemplate; this.ossOperator = ossOperator;
    }
    ...
    @Override @Transactional(rollbackFor = Exception.class) public ExplorePostVO create(ExplorePostSaveRequest request) {
        ...
        if (postMapper.insert(post) != 1) throw new BusinessException("探店内容发布失败，请稍后重试");
    ...
    private List<ExplorePostVO> postViews(List<ExplorePost> posts, Long currentUser) { if (posts.isEmpty()) return Collections.emptyList(); Set<Long> userIds = posts.stream().map(ExplorePost::getUserId).collect(Collectors.toSet()); Set<Long> shopIds = posts.stream().map(ExplorePost::getShopId).filter(item -> item != null).collect(Collectors.toSet()); Map<Long, User> users = users(userIds); Map<Long, Shop> shops = shops(shopIds); return posts.stream().map(item -> postView(item, currentUser, users, shops)).collect(Collectors.toList()); }
    private ExplorePostVO postView(ExplorePost post, Long currentUser, Map<Long, User> users, Map<Long, Shop> shops) { ExplorePostVO view = new ExplorePostVO(); view.setId(post.getId()); view.setUserId(post.getUserId()); User user = users.get(post.getUserId()); view.setAuthorName(user == null ? "校园用户" : user.getNickname()); view.setAuthorAvatar(user == null ? null : user.getAvatarUrl()); view.setShopId(post.getShopId()); Shop shop = shops.get(post.getShopId()); view.setShopName(shop == null ? "店铺已归档" : shop.getName()); view.setTitle(post.getTitle()); view.setContent(post.getContent()); view.setImages(splitImages(post.getImages())); view.setLikeCount(positive(post.getLikeCount())); view.setCommentCount(positive(post.getCommentCount())); view.setPostStatus(post.getPostStatus()); view.setCreatedAt(post.getCreatedAt()); view.setUpdatedAt(post.getUpdatedAt()); view.setLiked(currentUser != null && likeMapper.selectCount(Wrappers.<com.qinghe.life.entity.ExploreLike>lambdaQuery().eq(com.qinghe.life.entity.ExploreLike::getPostId, post.getId()).eq(com.qinghe.life.entity.ExploreLike::getUserId, currentUser)) > 0); return view; }
