...
@Data public class UserDTO {
    ...
    public Map<String, String> toMap() { Map<String,String> map = new HashMap<String,String>(); map.put("id", String.valueOf(id)); map.put("username", username == null ? "" : username); map.put("nickname", nickname == null ? "" : nickname); map.put("avatarUrl", avatarUrl == null ? "" : avatarUrl); map.put("phoneMasked", phoneMasked == null ? "" : phoneMasked); map.put("profileCompleted", String.valueOf(Boolean.TRUE.equals(profileCompleted))); map.put("hasPassword", String.valueOf(Boolean.TRUE.equals(hasPassword))); map.put("realName", realName == null ? "" : realName); map.put("studentNo", studentNo == null ? "" : studentNo); map.put("hasStudentProfile", String.valueOf(Boolean.TRUE.equals(hasStudentProfile))); return map; }
    public static UserDTO fromMap(Map<Object,Object> map) { UserDTO dto = new UserDTO(); dto.setId(Long.valueOf(value(map, "id"))); dto.setUsername(value(map, "username")); dto.setNickname(value(map, "nickname")); dto.setAvatarUrl(value(map, "avatarUrl")); dto.setPhoneMasked(value(map, "phoneMasked")); dto.setProfileCompleted(Boolean.valueOf(value(map, "profileCompleted"))); dto.setHasPassword(Boolean.valueOf(value(map, "hasPassword"))); dto.setRealName(value(map, "realName")); dto.setStudentNo(value(map, "studentNo")); dto.setHasStudentProfile(Boolean.valueOf(value(map, "hasStudentProfile"))); return dto; }
